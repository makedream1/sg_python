...
from app.models import db, User, Posts, Authors, lower
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:2367@localhost:5432/postgres'
...
def detail(id):
    ...
    """
    topic = Posts.query.filter_by(id=id).first_or_404()
    detail = {'id': id, 'title': topic.topic, 'text': topic.article_text, 'author': topic.author.author,
              'url': topic.url, 'price': topic.price, 'currency': topic.currency}
...
def home_page_post():
    ...
    if request.method == 'POST':
        ...
        if search_query:
            ...
            topics = Posts.query.join(Authors).filter(
                (Authors.author.ilike(like_str)) | (Posts.topic.ilike(like_str)))
